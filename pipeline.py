...
from shapelet_extraction import shapelet_discovery_mts
from shapelet_transform import shapelet_transform_mts
...
def train(X_train:np.array, config:dict, random_state:int=42, debug:bool=False):
    """
    ...
    train_generator = torch.utils.data.DataLoader(train_dataset, config["batch_size"], shuffle=True)

    loss_function = PNTripletLoss(config)
...
def classify_shapelets(X_train:np.ndarray, y_train:np.ndarray, X_test:np.ndarray, y_test:np.ndarray, config:dict, encoder:torch.nn.Module):
